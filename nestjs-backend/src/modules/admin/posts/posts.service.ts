...
export class PostsService {
  ...
  async list(filters: any = {}, perPage: number = 20, page: number = 1, relations: string[] = []) {
    return this.listGeneric(this.postRepository, filters, perPage, page, relations);
  }
  ...
  async create(createDto: any) {
    const { category_ids, tag_ids, ...postData } = createDto;
  ...
  async update(id: string, updateDto: any) {
    const { category_ids, tag_ids, ...postData } = updateDto;
