import { Injectable } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository, FindOptionsWhere, ILike, FindManyOptions } from 'typeorm';
import { PostCategory } from '../../../entities/post-category.entity';

@Injectable()
export class PostCategoriesService {
  constructor(
    @InjectRepository(PostCategory)
    private readonly postCategoryRepository: Repository<PostCategory>,
  ) {}

  // Generic list method
  private async list<T>(
    repository: Repository<T>,
    filters: any = {},
    perPage: number = 20,
    page: number = 1,
  ) {
    const validPerPage = Math.min(perPage, 100);
    const skip = (page - 1) * validPerPage;

    const where: FindOptionsWhere<T> = {};

    if (filters.status) {
      (where as any)['status'] = filters.status;
    }

    if (filters.search) {
      const searchFields = ['name', 'title', 'email'];
      for (const field of searchFields) {
        if (this.hasProperty(repository, field)) {
          (where as any)[field] = ILike(`%${filters.search}%`);
          break;
        }
      }
    }

    const findOptions: FindManyOptions<T> = {
      where,
      order: { created_at: 'DESC' } as any,
      skip,
      take: validPerPage,
    };

    const [data, total] = await repository.findAndCount(findOptions);

    return {
      data,
      meta: {
        total,
        per_page: validPerPage,
        current_page: page,
        last_page: Math.ceil(total / validPerPage),
        from: skip + 1,
        to: Math.min(skip + validPerPage, total),
      },
    };
  }

  // Check if entity has a specific property
  private hasProperty<T>(repository: Repository<T>, property: string): boolean {
    const metadata = repository.metadata;
    return metadata.columns.some(column => column.propertyName === property);
  }

  // Post Categories
  async getPostCategories(filters: any = {}, perPage: number = 20, page: number = 1) {
    return this.list(this.postCategoryRepository, filters, perPage, page);
  }

  async getPostCategory(id: string) {
    return this.postCategoryRepository.findOne({
      where: { id: Number(id) } as any,
    });
  }

  async createPostCategory(createPostCategoryDto: any) {
    const postCategory = this.postCategoryRepository.create(createPostCategoryDto as any);
    return this.postCategoryRepository.save(postCategory);
  }

  async updatePostCategory(id: string, updatePostCategoryDto: any) {
    const existingPostCategory = await this.postCategoryRepository.findOne({
      where: { id: Number(id) } as any,
    });

    if (!existingPostCategory) {
      return null;
    }

    const updatedPostCategory = this.postCategoryRepository.merge(existingPostCategory, updatePostCategoryDto as any);
    return this.postCategoryRepository.save(updatedPostCategory);
  }

  async deletePostCategory(id: string) {
    const existingPostCategory = await this.postCategoryRepository.findOne({
      where: { id: Number(id) } as any,
    });

    if (!existingPostCategory) {
      return null;
    }

    if (this.hasProperty(this.postCategoryRepository, 'deleted_at')) {
      (existingPostCategory as any).deleted_at = new Date();
      return this.postCategoryRepository.save(existingPostCategory);
    } else {
      return this.postCategoryRepository.remove(existingPostCategory);
    }
  }
}
