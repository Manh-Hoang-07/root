    ...
    if (tableExists) {
      try {
        ...
        if (!hasTimestampColumn || !hasNameColumn) {
          ...
          console.log('⚠️  Bảng migrations có cấu trúc sai. Đang xóa và tạo lại...');
          ...
          console.log('✅ Đã tạo lại bảng migrations với cấu trúc đúng.');
        } else {
          // Bảng đã đúng cấu trúc, không cần làm gì
          console.log('✅ Bảng migrations đã có cấu trúc đúng.');
          return;
        }
      } catch (error) {
        // Nếu có lỗi khi kiểm tra, xóa và tạo lại
        console.log('⚠️  Lỗi khi kiểm tra bảng migrations. Đang tạo lại...');
        await queryRunner.query('DROP TABLE IF EXISTS `migrations`');
        await this.createMigrationsTable(queryRunner);
      }
    } else {
      ...
      await this.createMigrationsTable(queryRunner);
      console.log('✅ Đã tạo bảng migrations (database mới).');
    }
  }
