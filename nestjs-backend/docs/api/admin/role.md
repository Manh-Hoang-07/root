...
```bash
curl -X GET "http://localhost:3000/admin/roles?page=1&limit=10" \
...
```bash
curl -X PUT http://localhost:3000/admin/roles/1 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE http://localhost:3000/admin/roles/1 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json"
```
...
```bash
curl -X POST http://localhost:3000/admin/roles/1/permissions \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
