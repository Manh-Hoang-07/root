      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="category in categories" :key="category.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ category.parent_name || 'Không có' }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ category.slug }}</td>
            <td class="px-6 py-4 whitespace-nowrap">
              <img v-if="category.image" :src="getImageUrl(category.image)" alt="Hình ảnh" class="h-10 w-10 object-contain" />
              <span v-else class="text-gray-400">Không có hình ảnh</span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <span 
...
async function fetchParentCategories() {
  try {
    const response = await axios.get(endpoints.categories.list, { params: { parent_only: true } })
...
async function createCategory(formData) {
  ...
  try {
    await axios.post(endpoints.categories.create, formData)
    await Promise.all([
      fetchCategories(),
      fetchParentCategories()
    ])
...
async function updateCategory(formData) {
  ...
  try {
    await axios.post(endpoints.categories.update(selectedCategory.value.id), formData)
    await Promise.all([
      fetchCategories(),
      fetchParentCategories()
    ])
