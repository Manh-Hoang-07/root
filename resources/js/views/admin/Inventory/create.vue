    ...
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium text-gray-900">Thêm tồn kho mới</h3>
          <button
            @click="onClose"
...
async function handleSubmit() {
  try {
    ...
    const response = await apiClient.post('/api/admin/inventory', form)
...
function resetForm() {
  Object.keys(form).forEach(key => {
    if (key === 'quantity' || key === 'reserved_quantity') {
      form[key] = 0
