    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="inventory in items" :key="inventory.id" class="hover:bg-gray-50">
...
const ImportInventory = defineAsyncComponent(() => import('./import.vue'))
const InventoryFilter = defineAsyncComponent(() => import('./filter.vue'))
...
const { 
  ...
  updateFilters, 
  deleteItem 
} = useDataTable(endpoints.inventory.list, {
  defaultFilters: {
    search: '',
    warehouse_id: '',
    brand_id: '',
...
async function loadFilterOptions() {
  try {
    ...
    showError('Không thể tải tùy chọn bộ lọc')
  }
}
...
async function handleInventoryCreated() {
  await fetchData()
  closeCreateModal()
  showSuccess('Tồn kho đã được tạo thành công')
}

async function handleInventoryUpdated() {
  await fetchData()
  closeEditModal()
  showSuccess('Tồn kho đã được cập nhật thành công')
}
...
async function deleteInventory() {
  try {
    await deleteItem(selectedInventory.value.id)
    closeDeleteModal()
    showSuccess('Tồn kho đã được xóa thành công')
  } catch (error) {
    showError('Không thể xóa tồn kho')
  }
}
